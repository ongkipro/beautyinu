...
console.log(`Starting update for ${pagesData.length} pages on store ${store}...\n`);
...
for (let i = 0; i < pagesData.length; i++) {
  ...
  const varsPath = '/tmp/shopify_page_update_vars.json';
  const queryPath = '/tmp/shopify_page_update_query.graphql';
  ...
  try {
    ...
    if (res.pageUpdate?.userErrors?.length > 0) {
      ...
      console.log(`  ✓ Successfully updated page: "${res.pageUpdate?.page?.title}" (id: ${res.pageUpdate?.page?.id})`);
    }
  } catch (err) {
...
// Save local copy of updated pages
...
// Clean up temp files
try {
  fs.unlinkSync('/tmp/shopify_page_update_vars.json');
  fs.unlinkSync('/tmp/shopify_page_update_query.graphql');
} catch (_) {}
